...
export default function withAuthGuard(Component: ComponentType) {
  return function AuthGuard() {
    const [state, setState] = useState<
      AuthenticationResult | Promise<AuthenticationResult> | Error | undefined
    >();
    useEffect(() => {
      const promise = client.reAuthenticate();
      setState(promise);
      promise.then(setState).catch((e) => setState(new Error(e)));
    }, []);
    if (state instanceof Promise) return <BrandLoading />;
